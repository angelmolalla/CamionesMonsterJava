...
<%@page import="ec.edu.espe.modelo1.XeopeOpeper"%>
...
<html lang=es>
    <head>
        ...
        <script>
            ...
            <p:forEach items="${opcion}" var="u">
                veri = 0;

                if ("${opcionPerfil.size()}" == 0)
                {
                    document.getElementById("keepRenderingSort").options[i] = new Option("${u.xeopcDesc}", "${u.xeopcCodigo}");
                    i++;
                } else
                {

                <p:forEach items="${opcionPerfil}" var="x">

                    if ("${u.xeopcCodigo}" == "${x.xeopcOpcion.xeopcCodigo}")
                    {
                        veri = 1;
                    }
                ...
                i = 0;
            <p:forEach items="${opcionPerfil}" var="x">
                alert("${x.xeperPerfil.xeperCodigo}");
                if ("${x.xeperPerfil.xeperCodigo}" == valor1)

                {

                    document.getElementById("keepRenderingSort_to").options[i] = new Option("${x.xeopcOpcion.xeopcDesc}", "${x.xeopcOpcion.xeopcCodigo}");
                    i++;
                }
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="page-header">
                            <h2>Opción Por Perfil</h2>
                        </div>
                        <form action="http://localhost:48823/CamionesMonsterMarcelo/OpcionPerfilConst" method="post">           
